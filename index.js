...
const PAUSE_CLASS_NAME = "pause";
const SHOW_CLASS_NAME = "show";
const HIDDEN_CLASS_NAME = "hidden";
const ME_CLASS_NAME = "me";
const HIDDEN_CURSOR_CLASS_NAME = "hidden-cursor";
...
function hiddenControls() {
    if (!enterControls && !pause) {
        window.fadeOut(videoControls, VIDEO_CONTROL_DURATION, videoBox, "mousemove").then(() => {
            controlsShow = false;
            window.execCallback(hideControlsCallbackArr);
        });
        videoBox.classList.add(HIDDEN_CURSOR_CLASS_NAME);
    }
}
...
function enterOrExitControlTipInitial(tipBoard, enterEle, exitEle, enterTip) {
    ...
    let showTipBoard = (causeEle) => {
        window.fadeIn(tipBoard, VIDEO_CONTROL_DURATION, causeEle).then(() => {
        });
    }

    let hiddenTipBoard = (causeEle) => {
        window.fadeOut(tipBoard, VIDEO_CONTROL_DURATION, causeEle).then(() => {
        });
    }
...
function videoBarrageInitial() {
    ...
    videoContainer.classList.remove(HIDDEN_CLASS_NAME);
}
