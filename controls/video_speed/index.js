...
function showSpeedList() {
    window.fadeIn(speedList, VIDEO_CONTROL_DURATION, speedContainer).then(() => {});
}

function hideSpeedList() {
    window.fadeOut(speedList, VIDEO_CONTROL_DURATION, speedContainer).then(() => {});
}
...
function changeVideoSpeed(e) {
    ...
    let activeSpeedItem = speedList.querySelector(`.speed-item.${ACTIVE_CLASS_NAME}`);
    activeSpeedItem.classList.remove(ACTIVE_CLASS_NAME);
...
(function () {
    ...
    speedValues.forEach((speed) => {
        ...
        if (speed === commonSpeed) {
            speedItemClone.classList.add(ACTIVE_CLASS_NAME);
        }
